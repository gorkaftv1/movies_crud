...
export default function EditPlaylistPage({ params }: PageProps) {
  ...
  // Obtener parámetros y configurar playlist ID
  useEffect(() => {
  ...
  const loadPlaylist = async () => {
    try {
      ...
      const playlist = await getPlaylistById(supabase, playlistId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updated = await updatePlaylist(supabase, playlistId, user.id, updateData);
  ...
  if (authLoading || loadingPlaylist || !playlistId) {
    return (
