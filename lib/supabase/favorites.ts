...
export async function getMoviesWithFavorites(): Promise<{ data: MovieWithFavorite[] | null; error: string | null }> {
  try {
    ...
    // Transformar los datos para incluir información de favoritos
    const moviesWithFavorites: MovieWithFavorite[] = (data || []).map(movie => ({
      id: movie.id,
      title: movie.title,
      year: movie.year,
      ...
      is_favorited: movie.user_favorites.length > 0
    }));
